...
def build_context(data: Dict[str, Any]) -> Tuple[Dict[str, str], Dict[str, str]]:
    ...
    flags: Dict[str, str] = {
        ...
        "CONTRIBUTING_FACTOR_ON_PR_ADVERSE": yn(data.get("contributing_factor_on_pr_adverse")),
        "CONTRIBUTING_FACTOR_ON_PR_CLIENT": yn(data.get("contributing_factor_on_pr_client")),
        "ADVERSE_ISSUED_CITATION": yn(data.get("adverse_issued_citation")),
    ...
    ctx: Dict[str, str] = {
        ...
        "COLLISION OCCURRENCE": data.get("collision_occurrence", ""),
        "PAGE_OF_PR_NARRATIVE": data.get("page_of_pr_narrative", ""),
        "PAGE_OF_DIAGRAM": data.get("page_of_diagram", ""),

        "ADVERSE_CONTRIBUTING_ACTION_1": data.get("adverse_contributing_action_1", ""),
        ...
        # ── BOTTOM-specific / LOU fields (typed directly by the agent in this test build) ──
...
def make_arb_packet(data: Dict[str, Any], out_dir: str | Path = "output",
    ...
    top_path = out_dir / f"{file_prefix}_TOP.docx"
    bottom_path = out_dir / f"{file_prefix}_BOTTOM.docx"
