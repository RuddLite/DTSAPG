...
## Pre-fill from a completed demand letter

Agents can upload a completed demand letter (.docx, produced by the
existing demand-letter-generator project) at the top of the dashboard.
`demand_letter_import.py` regex-matches the letter's fixed sentence
structure (see `DemandTemplate_NY.docx`) to pull out Property Damage, Loss
of Use, Towing, Repair Hours, Raw Days, Total Days, Applicable Weekend
Days, and the daily-rate breakdown (Daily Replacement Value, Driver Wage
hourly/daily, Final Daily Rate) — then fills the matching dashboard fields
client-side via `POST /prefill`. Nothing is auto-submitted: every field
lands in the normal input for the agent to review, edit, or clear, exactly
like the rest of the dashboard.

This is a best-effort text match against sentence wording that's fixed
today — if the demand letter template's phrasing changes, update the
regexes in `demand_letter_import.py` to match. Fields it doesn't recognize
are simply left blank; nothing is guessed.
