...
Field dict keys:
    ...
    type        – "text" | "textarea" | "number" | "select" | "yesno" | "state"
    section     – "TOP" or "BOTTOM"  (drives the two-stage wizard order)
    ...
    clause_preview – for type="yesno" fields that gate an optional clause:
                  the literal clause text shown to the agent so they know
                  what selecting "Yes" will add to the document. Tokens are
                  shown in {curly braces} as a hint of what the agent will
                  be asked to fill in.
"""
...
FIELDS: List[Dict[str, Any]] = [
    ...
    {"id": "accident_state", "label": "Accident State", "type": "state",
     ...
     "help": "Two-letter state where the accident occurred. Determines the BOTTOM case law inserted."},
    {"id": "date_of_loss", "label": "Date of Loss", "type": "text",
     "section": "TOP", "required": True, "help": "MM/DD/YYYY"},
    ...
    {"id": "lane_of_travel", "label": "Recovering Party Lane of Travel", "type": "text", "section": "TOP", "required": True},
    {"id": "direction_of_travel", "label": "Recovering Party Direction of Travel", "type": "text", "section": "TOP", "required": True},
    {"id": "road", "label": "Road", "type": "text", "section": "TOP", "required": True},
    {"id": "adverse_lane_of_travel", "label": "Adverse Party Lane of Travel", "type": "text", "section": "TOP", "required": True},
    {"id": "adverse_direction_of_travel", "label": "Adverse Party Direction of Travel", "type": "text", "section": "TOP", "required": True},
    {"id": "collision_occurrence", "label": "Collision Occurrence", "type": "textarea",
     "section": "TOP", "required": True, "help": "One or two sentences describing how the impact happened."},
    {"id": "page_of_pr_narrative", "label": "Page of PR Narrative", "type": "text", "section": "TOP", "required": True},
    {"id": "page_of_diagram", "label": "Page of PR Scene Diagram", "type": "text", "section": "TOP", "required": True},

    # ── driver / supervisor statement (4-way combo clause) ──
    ...
    {"id": "adverse_contributing_action_1", "label": "Adverse Contributing Action #1", "type": "text",
     "section": "TOP", "required": True, "help": "Listed under NEGLIGENCE THEORY, alongside \"Inattention\"."},
    {"id": "adverse_contributing_action_2", "label": "Adverse Contributing Action #2 (optional)", "type": "text",
    ...
    {"id": "contributing_factor_on_pr_adverse", "label": "Contributing Factor Cited Against Adverse on PR?", "type": "yesno",
     "section": "TOP", "required": True,
    ...
    {"id": "contributing_factor_on_pr_client", "label": "Contributing Factor Cited Against Recovering Party (Client) on PR?", "type": "yesno",
     "section": "TOP", "required": True,
    ...
    {"id": "adverse_issued_citation", "label": "Was Adverse Party Issued a Citation?", "type": "yesno",
     "section": "TOP", "required": True,
    ...
    {"id": "pr_page_showing_adverse_point_of_impact", "label": "PR Page — Adverse Point of Impact", "type": "text", "section": "TOP", "required": True},
    {"id": "adverse_point_of_impact", "label": "Adverse Party Point of Impact", "type": "text", "section": "TOP", "required": True},
    {"id": "pr_page_showing_client_point_of_impact", "label": "PR Page — Client Point of Impact", "type": "text", "section": "TOP", "required": True},
    ...
    {"id": "photos_of_damages", "label": "Photos of Damages Available?", "type": "yesno",
     "section": "TOP", "required": True,
     "clause_preview": "The points of impact and photos prove that Recovering Party was there to be seen, and Adverse Party had the last opportunity to avoid this collision."},
    {"id": "liability_obvious", "label": "Is Liability Obvious?", "type": "yesno",
     "section": "TOP", "required": True,
     "clause_preview": "The points of impact and photos also prove that the Adverse Party's actions left the Recovering Party without the opportunity to take evasive action and that Adverse Party was the sole cause of this collision."},
    {"id": "favorable_pr", "label": "Favorable Police Report?", "type": "yesno",
     "section": "TOP", "required": True,
     "clause_preview": "Police investigated and narrated their findings in a formal report... The report was clear and cited the Adverse Party's {ADVERSE_CONTRIBUTING_ACTION_1} as the cause of this loss."},
    ...
    {"id": "adverse_paid_partial_lou_without_supports", "label": "Did Adverse Pay a Partial LOU Amount Without Support?", "type": "yesno",
     "section": "TOP", "required": True,
     "clause_preview": "If Adverse Party wishes to pursue a lesser amount for loss of use, they must support the reduction through their own independent evidence and logic. Simply refunding an arbitrary amount does not exempt Adverse Party from their burden of proof."},
    ...
    # ── loss of use total (TEST BUILD: typed by the agent, not calculated) ──
    {"id": "total_lou_days", "label": "Total Loss-of-Use Days", "type": "number",
     "section": "TOP", "required": True,
     "help": "Used for both the TOP ([TOTAL_LOU_DAYS]) and BOTTOM ([TOTAL_DAYS]) documents — enter once here."},
    {"id": "loss_of_use_amount", "label": "Loss of Use Amount", "type": "number",
     "section": "TOP", "required": True,
     "help": "Used for both the TOP and BOTTOM documents, and included in the Grand Total."},
    ...
    # ═══════════════════════════ BOTTOM — LOU / DAMAGES ═══════════════════
    # ── TEST BUILD: these were previously auto-calculated from the demand
    #    letter generator's state/vehicle rate tables. For this test build,
    #    the agent enters each value directly. ──
