...
from flask import Flask, render_template, request, send_file, Response, abort, redirect, url_for, jsonify
from arb_packet_generator import make_arb_packet, states_with_case_law
from demand_letter_import import extract_from_demand_letter
...
# ─── prefill from an uploaded, completed demand letter ─────────────────────
_PREFILLABLE_FIELD_IDS = {f["id"] for f in FIELDS}  # only ever return known field ids

@app.route("/prefill", methods=["POST"])
@protect
def prefill():
    upload = request.files.get("demand_letter")
    if not upload or not upload.filename:
        abort(400, "No file uploaded.")
    if not upload.filename.lower().endswith(".docx"):
        abort(400, "Please upload a .docx file.")
    try:
        extracted = extract_from_demand_letter(upload.stream)
    except Exception:
        abort(400, "Could not read that file as a Word document. "
                    "Please upload the completed demand letter .docx.")
    # belt-and-suspenders: only ever hand back ids that are real dashboard fields
    extracted = {k: v for k, v in extracted.items() if k in _PREFILLABLE_FIELD_IDS}
